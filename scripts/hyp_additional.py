"""This script tests additional hypothesis beyond the max delChi2/SNR.
The script requires maskname, boolean of whether blue or red mask, 
slit idx, and nthidx which refers to the nth max negative delChi2data 
...
#MAKE SURE THAT SAME VERSION OF HYPOTHESIS_TESTER IS MAINTAINED WITH hyp_test.py
def hypothesis_tester(input_data_dir, maskname, slit_idx, z, widths, \
		...
		dat, err, header = extract_single_data(data_err, list_headers, slit_idx)
		if(maskred_flag == True):
			...
			dat_red, _, _ = extract_single_data(data_err, list_headers, slit_idx)
...
#User inputs in sequential order: maskname, blue/red flag, slit_idx, nthidx
maskname = sys.argv[1]
flag = (sys.argv[2].lower() == 'true') #user entered input whether bluer or redder mask
slitidx = np.int(sys.argv[3])
nthidx = np.int(sys.argv[4])
...
z_bestidx, w_bestidx = bestmodels[slitidx-1]

#find nth negative delChi2data max
flattened = np.copy(delChi2data[slitidx].flatten())
flattened_nthindex = np.argsort(flattened)
flattened_sorted = flattened[flattened_nthindex]

#find nth max index

w_nthidx, z_nthidx = np.where(np.isin(delChi2data[slitidx], 
	flattened_sorted[nthidx]))
z_nthidx = z_nthidx[0]
w_nthidx = w_nthidx[0]

#First check if SNR is greater than 10; if they are 
...
if(SNRdata[slitidx, w_nthidx, z_nthidx] > 7.):
	...
	delChi2data[slitidx, w_nthidx, z_nthidx]) > 1.):
		if(np.abs(zdata[z_bestidx] - zdata[z_nthidx]) > .01):
			fname, fig = hypothesis_tester(input_data_dir, maskname, slitidx, zdata, widthsdata, \
					image, ivar, wg, z_nthidx, w_nthidx, relative_strength, fudge_factor, flag)
		
			#Ask user whether to save the fig
			savefig_flag = input("Save fig? ").lower().strip()
			...
			print("z closer than 0.01.")
	else:
		print("delChi2data closer than 1.")
else:
	print("SNR lower than 10.")